...
import com.yxbuild.dataMode.{CityInfo, ProductInfo, UserInfo, UserVisitAction}
import com.yxbuild.utils.{RanOpt, RandomDate, RandomNum, RandomOptions}
...
object MockerOffline {
  ...
  /**
    * 执行Spark SQL，将数据保存到Hive
    *
    * @param sparkSession SparkSession对象
    * @param tableName 数据表名称
    * @param dataFrame DataFrame对象
    */
  ...
  def mockUserAction(): List[UserVisitAction] = {
    ...
    val startDate: Date = new SimpleDateFormat("yyyy-MM-dd").parse("2018-11-26")
    val endDate: Date = new SimpleDateFormat("yyyy-MM-dd").parse("2018-11-27")
